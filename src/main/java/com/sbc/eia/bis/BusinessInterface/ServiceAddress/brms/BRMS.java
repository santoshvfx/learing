// $Id: BRMS.java,v 1.6 2007/10/15 20:50:04 gg2712 Exp $
 ...
 */
public class BRMS extends Host implements ServiceAddressLocationI 
{
	...
	protected RetrieveLocResp doServiceAddressValidation(
		...
		throws
			...
			ObjectNotFound 
	{
		maxAddrSize = maxAddressLimit;
		maxUnitSize = maxUnitLimit;
...
//        	getLimProps().getProperty("INITIAL_CONTEXT_PROPERTIES_FILE"));
		...
		throws InvalidStateException, NullDataException, InvalidCompanyException 
	{
		...
		return new Selector[] {
			new Selector(new Company(Company.Company_SouthernNETelephone,
				new State(State.State_Connecticut),	null,null),ServiceAddressLocationI.LOCATION_INTERFACE_NAME,
				(BRMS.class).getName()),};
	}
		   ...
		   ObjectNotFound
	{
		...
		try 
		{
			...
			request = new RetrieveLocReq (getLimBase().limBase, ahBrmsReq, aLocationPropertiesToGet, aPreviousOwnerName);
			...
			exactMatchReq = (aMaximumBasicAddressReturnLimit == LIMTag.EXACT_MATCH_REQ
					? true: false);
			...
			result = (doServiceAddressValidation(request,
				aMaximumBasicAddressReturnLimit,
				aMaximumUnitReturnLimit)).toServiceAddressReturn();
	...
	throws
        ...
        try 
        {
            ...
            getUtility().log (LogEventId.AUDIT_TRAIL, "BRMS::retrieveLocationForTelephoneNumber()|BRMS::getAddressByTN()|PRE");
            Location smLocation = (Location) getAddressByTN(service);
            getUtility().log (LogEventId.AUDIT_TRAIL, "BRMS::retrieveLocationForTelephoneNumber()|BRMS::getAddressByTN()|POST");
            ...
            request = new RetrieveLocReq(getLimBase().limBase, ahBrmsReq, aLocationPropertiesToGet);
            request.logAddressReq();
            ...
            getUtility().log (LogEventId.AUDIT_TRAIL,
                "BRMS::retrieveLocationForTelephoneNumber()|BRMS::doAddressValidation()|PRE");
            ...
            getUtility().log (LogEventId.AUDIT_TRAIL,
                "BRMS::retrieveLocationForTelephoneNumber()|BRMS::doAddressValidation()|POST");

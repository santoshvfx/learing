// $Id: PREMIS.java,v 1.11 2006/06/09 17:44:36 ak7627 Exp $
 ...
 * @author: David Prager
 */
public class PREMIS extends Host implements LocationI
{
	...
	private String premisOverride = null;
	*/
    /**
    ...
    /**
     * Send the AddressValidation request to PREMISServer and get the response.
     ...
     */
    protected RetrieveLocResp doAddressValidation( PremisRetrieveLocReq request,
    	LongOpt maxAddressLimit, LongOpt maxUnitLimit, List sagaList )
    	throws ServiceException, InvalidData, AccessDenied, BusinessViolation, DataNotFound, SystemFailure, NotImplemented, ObjectNotFound
    {
        ...
        */
    	...
    	if (response instanceof PremisTNMenu){
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handleTNMenu()|PRE");
    		response = handleTNMenu(request,(PremisTNMenu) response);
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handleTNMenu()|POST");
        }
    	else if (response instanceof PremisSagaResp){
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handleSagaMenu()|PRE");
    		response = handleSagaMenu(request,(PremisSagaResp) response);
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handleSagaMenu()|POST");
        }
    
        if (response instanceof PremisHit){
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handlePremisHit()|PRE");
    		handlePremisHit(request, (PremisHit) response);
            getUtility().log(LogEventId.AUDIT_TRAIL,"PREMIS::doAddressValidation()|PREMIS::handlePremisHit()|POST");
        }
    	...
    	throws InvalidStateException, NullDataException, InvalidCompanyException
    {
    	...
    	return new Selector[] {
    		new Selector(new Company(Company.Company_PacificBell, new State(State.State_California), null, null),
    			LocationI.LOCATION_INTERFACE_NAME, (PREMIS.class).getName()),
    ...
    */ 
    		...
    		if (request.getAddr().getPostalCode() == null
    			...
    			temp =
    				new PremisRetrieveLocReq(
                        getLimBase().limBase, 
                        request.getTn(),
                        request.getProviderLocationPropertiesToGet());
    		else
    			temp =
    				new PremisRetrieveLocReq(
    					getLimBase().limBase,
    ...
    public RetrieveLocationForAddressReturn retrieveLocationForAddress(
    	...
    	try {
    		...
    		PremisRetrieveLocReq request =
    			new PremisRetrieveLocReq(getLimBase().limBase, addrPremis, aPropertiesToGet);
            ...
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForAddress()|PREMIS::doAddressValidation()|PRE");
    		result =
    			(doAddressValidation(request,
    				aMaximumBasicAddressReturnLimit,
    				aMaximumUnitReturnLimit,
                    getSagaListByZipCode(addrPremis.getPostalCode())))
    				.toAddressReturn();
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForAddress()|PREMIS::doAddressValidation()|POST");
    	} 
    	...
    	throws InvalidData, AccessDenied, BusinessViolation, DataNotFound, SystemFailure, NotImplemented, ObjectNotFound
    {
    	...
    	try
    	{
    		...
    		PremisRetrieveLocReq  request  = new PremisRetrieveLocReq(getLimBase().limBase, aService, aPropertiesToGet);
            ...
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForService()|PREMIS::doAddressValidation()|PRE");
    		RetrieveLocResp response = 
                    doAddressValidation( request,
    			                         aMaximumBasicAddressReturnLimit,
                                         aMaximumUnitReturnLimit,
                                         null);
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForService()|PREMIS::doAddressValidation()|POST"); 
            ...
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForService()|PREMIS::toServiceReturn()|PRE");
    		result = response.toServiceReturn();
            getLimBase().limBase.log(LogEventId.AUDIT_TRAIL, "PREMIS::retrieveLocationForService()|PREMIS::toServiceReturn()|POST");
    	}
    	catch (ServiceException e)
    	{
    		handleServiceException(e);

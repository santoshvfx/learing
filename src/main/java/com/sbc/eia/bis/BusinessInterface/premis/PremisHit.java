// $Id: PremisHit.java,v 1.19 2006/01/11 00:19:09 jd3462 Exp $
...
/**
 * This class represents an address-match from PREMISServer.
 * Creation date: (3/12/01 9:37:38 AM)
 * @author: David Prager
 */
public class PremisHit extends AddrMatchResp
{
     ...
     */
    public PremisHit(LIMBase utility, PremisHITResp_t result, PremisRetrieveLocReq aRequest)
    	throws SystemFailure
    {
        ...
        if ((result.HITPktResp.RTCD.startsWith("301")) ||
        	(result.HITPktResp.RTCD.startsWith("308")))   
     ...
     * considered a hit because it includes a single address, although with multiple units.
    ...
    /**
     * Construct and store an ProviderLocationPropertySeq from the ASON Hit data.
     * Creation date: (3/7/01 10:02:01 AM)
     */
    protected void setProviderLocationPropertySeq()
    {
        ...
        propertyBuilder.setExchangeCode(fal.SagInfo.EXCH_ID);
        propertyBuilder.setPrimaryDirectoryCode(fal.SagInfo.DIR_CD);
        propertyBuilder.setStreetAddressGuideArea(fal.SAGA);
        propertyBuilder.setTarCode(fal.SagInfo.TAX_AREA_CD);
        propertyBuilder.setWorkingServiceOnLocation((workingTnSet.size() > 0) ? "true" : "false");
	        ...
	        fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase(State.State_Texas))
	    {
		    ...
		    if (fal.RmkData != null	&& fal.RmkData.length > 0
		    &&  fal.RmkData[0].BASC_ADDR_RMK != null 
		    &&  fal.RmkData[0].BASC_ADDR_RMK.toUpperCase().indexOf(PREMISTag.OWNED_WIRING) != -1)
            {
			    yn = PREMISTag.YES;
            }
		    propertyBuilder.setOwnedWiring(yn);
	    }

        // set Exco
        if ((fal.SagInfo.EXCH_ID != null && fal.SagInfo.EXCH_ID.length() > 0) &&
            (fal.PrmAddr.BascAddrInfo.STATE_CD != null)                       &&
            (fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_California ) || 
             fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Nevada ))) 
        {
            StringBuffer exco = new StringBuffer(fal.SagInfo.EXCH_ID.substring(0,Math.min(3,fal.SagInfo.EXCH_ID.length())));
            while (exco.length() < 3)
                exco.append(" ");
            if ( fal.SagInfo.CO_ID.length() > 0)
                exco.append(fal.SagInfo.CO_ID.substring(0,Math.min(2,fal.SagInfo.CO_ID.length())));
            propertyBuilder.setExco(exco.toString());
        }
        ...
        if (fal.SagInfo.LATA_PREMIS != null && fal.SagInfo.LATA_PREMIS.length() > 0)
        {
            ...
            {
                propertyBuilder.setLataCode(aLata);
            }
            else
            {
                utility.log(LogEventId.INFO_LEVEL_1, "Warning: LATA Value<" + fal.SagInfo.LATA_PREMIS.trim() + "> key not found in LATA.properties file" );
            }
        }

       // set LocalProviderServingOfficeCode
        if (fal.SagInfo.NPA_LST != null && fal.SagInfo.NPA_LST.length > 0  &&
            fal.SagInfo.TERMN_TRAF_AREA_CD != null)
        {
            propertyBuilder.setLocalProviderServingOfficeCode(fal.SagInfo.NPA_LST[0] + fal.SagInfo.TERMN_TRAF_AREA_CD);
        // set PrimaryNpaNxx
            if ((fal.PrmAddr.BascAddrInfo.STATE_CD != null) &&
                (fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Arkansas ) || 
                 ...
                 fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Oklahoma ) || 
                 fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Texas )))
            { 
                propertyBuilder.setPrimaryNpaNxx(validNpaNxx (fal.SagInfo.NPA_LST[0] + fal.SagInfo.TERMN_TRAF_AREA_CD));
            } 
            else
            { 
                propertyBuilder.setPrimaryNpaNxx(fal.SagInfo.NPA_LST[0] + fal.SagInfo.TERMN_TRAF_AREA_CD);
            } 
        }
        ...
        if ( fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_California )) {    
            for (int i = 0; i < fal.LnData.length; i++)
            {
                ...
                if (mwi != null && mwi.length() >= 12
                &&  Character.isDigit(mwi.charAt(0))
                &&  Character.isDigit(mwi.charAt(1))
                &&  Character.isDigit(mwi.charAt(2)))
                {
            ...
            if ( tempTN.length() > 0 )
            {
                propertyBuilder.setQuickDialTone(PREMISTag.YES);
                propertyBuilder.setQuickDialToneNumber(tempTN); 
            }
            else
            {
                propertyBuilder.setQuickDialTone(PREMISTag.NO);
            }    
        }      
        ...
        if ((fal.PrmAddr.BascAddrInfo.STATE_CD != null) &&     
          (fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_California ) || 
           fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Nevada )))
        { 
            propertyBuilder.setCommunityName(fal.ALT_CMTY_1_NM);
        }
        ...
        { 
            ...
            propertyBuilder.setServingAreaDescription(servingArea.toString());
        }
        ...
        if ((fal.PrmAddr.BascAddrInfo.STATE_CD != null) &&
            (fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Arkansas ) || 
             ...
             fal.PrmAddr.BascAddrInfo.STATE_CD.equalsIgnoreCase( State.State_Texas ))) 
        { 
            propertyBuilder.setRateZone(fal.SagInfo.RT_ZONE_CD); 
        } 
    ...
    protected void addWorkingTn(SuppLnInfo_t suppLnInfo)
    {
    	...
    	{
    		addWorkingTn(suppLnInfo.NpaPrfxLn.NPA,
    			suppLnInfo.NpaPrfxLn.PRFX_CD,
    			suppLnInfo.NpaPrfxLn.LN);
    	}
    }
    /**
    ...
    /**
     * returns Application PREMIS response structure.
     * Creation date: (3/26/02 12:15:44 PM)
    ...
    /**
     * Getter method for the string servingArea.
     * Creation date: (5/21/02 10:00:33 AM)
     * @return String
     * @see #setServingArea
     */
    ...
    {
    	return (
    		fal.PrmAddr.BascAddrInfo.StNbrId.SAD_HOUS_PRFX +
    		fal.PrmAddr.BascAddrInfo.StNbrId.SAD_HOUS_NBR +
    		fal.PrmAddr.BascAddrInfo.StNbrId.SAD_HOUS_NBR_SUFX +
    		fal.PrmAddr.BascAddrInfo.ASGND_HOUS_NBR_ID +
    		fal.PrmAddr.BascAddrInfo.StNm.SAD_ST_DRCTL +
    		fal.PrmAddr.BascAddrInfo.StNm.SAD_ST_NM +
    		fal.PrmAddr.BascAddrInfo.StNm.SAD_THRFR +
    		fal.PrmAddr.BascAddrInfo.StNm.SAD_ST_SUFX +
    		fal.PrmAddr.BascAddrInfo.CMTY_NM +
    		fal.PrmAddr.BascAddrInfo.STATE_CD +
    ...
    protected AddressHandlerPremis getServiceAddress()
    {
    	...
    	return(new AddressHandlerPremis(
    ...
    /**
     * Setter method for the string servingArea.
     * Creation date: (5/21/02 10:00:33 AM)
    ...
    public String toString()
    {
        ...
        try { sb.append(nl + "CommunityName[" + addressData.aCommunityName.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}
        ...
        try { sb.append(nl + "Exco[" + addressData.aExco.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}
        ...
        try { sb.append(nl + "OwnedWiring[" + addressData.aOwnedWiring.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}

        try { sb.append(nl + "PrimaryDirectoryCode[" + addressData.aPrimaryDirectoryCode.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}
        ...
        try { sb.append(nl + "QuickDialTone[" + addressData.aQuickDialTone.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}

        try { sb.append(nl + "QuickDialToneNumber[" + addressData.aQuickDialToneNumber.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}

        try { sb.append(nl + "RateZone[" + addressData.aRateZone.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}

        try { sb.append(nl + "ServingAreaDescription[" + addressData.aServingAreaDescription.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}

        try { sb.append(nl + "StreetAddressGuideArea[" + addressData.aStreetAddressGuideArea.theValue() + "]"); }
        catch (org.omg.CORBA.BAD_OPERATION e) {}
        catch (java.lang.NullPointerException e) {}
    ...
    /**
     * validNpaNxx contains a list of Fictitious NPA-TTA (first column) and a list of Valid NPA-NXX (second column)
     * for Southwest region. If the Fictitious NPA-TTA was found, the function returns the Valid NPA-NXX.

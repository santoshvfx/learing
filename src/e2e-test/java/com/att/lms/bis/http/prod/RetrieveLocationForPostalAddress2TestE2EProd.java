...
@TestPropertySource(locations = { "classpath:application-e2e-test-prod.properties", "classpath:application-e2e-real.properties" })
public class RetrieveLocationForPostalAddress2TestE2EProd extends AbstractControllerE2E {
...
//            "[{\"aContext\":{\"aProperties\":[{\"aTag\":\"Application\",\"aValue\":\"LIM_BIS\"},{\"aTag\":\"CustomerName\",\"aValue\":\"LIM_BIS\"}]},\"aExceptionData\":{\"aCode\":\"LIM-NotImplemented-00080\",\"aDescription\":\"retrieveLocationForPostalAddress2 is not implemented.\",\"aOrigination\":{\"___theValue\":\"LIM\",\"__discriminator\":true,\"__uninitialized\":false},\"aSeverity\":{\"___theValue\":{\"__value\":1},\"__discriminator\":true,\"__uninitialized\":false}}}]";

        BisContext aBisContext = getBisContext("LIM_BIS", "LIM_BIS");

        Address aAddress = new Address();
        LongOpt aMaxAddressAlternatives = new LongOpt();
        LongOpt aMaxCassCharPerLine = new LongOpt();

        RetrieveLocationForPostalAddress2RequestDto request = RetrieveLocationForPostalAddress2RequestDto.builder()
                .aBisContext(aBisContext)
                .aAddress(aAddress)
                .aMaxAddressAlternatives(aMaxAddressAlternatives)
                .aMaxCassCharPerLine(aMaxCassCharPerLine)
                .build();
        ...
        String responseBody = given(documentationSpec)
                ...
                .filter(document("retrieveLocationForPostalAddress2-prod",
                ...
                .post(StringUtils.join(DEFAULT_SPRING_BOOT_BASE_URI,
                        BisApi.LIM_BASE_URL,
                        "/",
                        LimController.RETRIEVE_LOCATION_FOR_POSTAL_ADDRESS_2_PATH_NAME))
                .then()
                .statusCode(500)
                .extract()
                .response()
...
//        assertThat(cleanResponse(responseBody)).isEqualTo(EXPECTED_RESPONSE);
    ...
    private FieldDescriptor[] getRequestFieldDescriptor() {
        return new FieldDescriptor[] {
                ...
                subsectionWithPath("aMaxCassCharPerLine").description("Max Cass Char Per Line"),
